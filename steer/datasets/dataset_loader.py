...
class DatasetLoader:
    ...
    def format_single_item(self, 
        ...
        for standard_key, custom_key in format_mapping.items():
            if custom_key == "":
                formatted_data[standard_key] = ""
            elif custom_key in data:
...
def is_memory_sft_dataset(dataset_name: str) -> bool:
    return dataset_name.startswith(("pb_", "cf_"))
...
def prepare_train_dataset(hparams):
    ...
    for dataset_name in dataset_names:
        ...
        if is_memory_sft_dataset(dataset_name):
            required_fields = ("question", "matching", "not_matching")
            for idx, item in enumerate(loaded_dataset):
                missing_fields = [field for field in required_fields if field not in item]
                assert not missing_fields, (
                    f"Memory SFT dataset {dataset_name} sample {idx} is missing required fields: {missing_fields}. "
                    f"Available fields: {sorted(item.keys())}"
                )
                item["matching"] = str(item["matching"])
                item["not_matching"] = "" if item["not_matching"] is None else str(item["not_matching"])

            if loaded_dataset:
                sample = loaded_dataset[0]
                print("[Memory SFT DEBUG] First formatted training sample:")
                for key in ("id", "case_id", "query", "target", "target_len"):
                    if key in sample:
                        print(f"  {key}: {sample[key]}")
                print(f"  question: {sample['question']}")
                print(f"  matching: {sample['matching']}")
                print(f"  not_matching: {sample['not_matching']!r}")
                if "target_true" in sample:
                    print(f"  target_true(metadata_only): {sample['target_true']}")
