import os
import torch
from .apply_memft_hparam import ApplyMemFTHyperParams
...
def reset_memft_layers(model, layers):
    """Reset only the MemFT activations for specified layers"""
    model = model.model
    for layer in layers:
        if hasattr(model, 'model') and (hasattr(model.model, 'layers') or (hasattr(model.model, 'module') and hasattr(model.model.module, 'layers'))):
            if isinstance(model.model, Hack_no_grad):
                model.model.module.layers[layer].reset(method_name="memft")
            else:
                model.model.layers[layer].reset(method_name="memft")
            ...
            if isinstance(model.transformer, Hack_no_grad):
                model.transformer.module.h[layer].reset(method_name="memft")
            else:
                model.transformer.h[layer].reset(method_name="memft")
        else:
            raise NotImplementedError("Failed to reset MemFT activations")

def apply_memft(hparams: ApplyMemFTHyperParams, pipline=None, vector=None):
    ...
    if pipline is None:
        ...
        model = pipline
    print('Apply MemFT to model: {}'.format(hparams.model_name_or_path))
    # Reset only MemFT activations for specified layers
    reset_memft_layers(model, hparams.layers)
    ...
    for layer, multiplier in zip(layers, multipliers):
        ...
        model.set_intervention(layer, intervention, "memft")
