...
from steer.utils.templates import model_supports_system_prompt, safe_apply_chat_template
...
def get_prefix_length(tokenizer, common_prefix=None):
    if common_prefix is None:
        ...
        tokens_a = safe_apply_chat_template(tokenizer, message_a, tokenize=True)
        tokens_b = safe_apply_chat_template(tokenizer, message_b, tokenize=True)
        prefix_length = 0
...
def prepare_groups(
    ...
    if use_chat_template:
        ...
        #     user_content = item[column_name]
        #     if column_name == "question" and {"query", "target", "target_len"}.issubset(item):
        #         name = str(item["query"]).rstrip(":").strip()
        #         user_content = f"What is the phone number of {name}?"
        def apply_input_template(item, column_name):
            messages = system_messages + [{"role": "user", "content": item[column_name]}]
            tokens = safe_apply_chat_template(tokenizer, messages, tokenize=True, add_generation_prompt=True)
