...
from .generate_memft_hparams import MemFTHyperParams
from .utils import get_prefix_length, load_state, prepare_groups, save_state
from steer.trainer.MemFTTrainer import MemFTTrainer


def generate_memft(args: MemFTHyperParams, dataset, model = None, dataset_name = None):
    """
    ...
    for layer in args.layers:
        ...
        output_dir = os.path.join(
            ...
            f"memft_{args.intervention_method}"
        )
        ...
        for concept_key, concept_groups in concept_grouped_data:
            ...
            print(f"[INFO] Start to train MemFT for concept: {concept_key_str}")
            ...
            print(f"[INFO] Using concept '{concept}' to train MemFT")
            
            # Store concept info for multi-concept datasets
            
            
            # Init the trainer
            benchmark_model = MemFTTrainer(
                model=model,
                layers=[layer],
                hparams=args 
            )
    ...
    print(f"[INFO] Generated MemFT {args.intervention_method} : {all_vectors}")
    if is_multi_concept:
        return all_vectors
    else:
