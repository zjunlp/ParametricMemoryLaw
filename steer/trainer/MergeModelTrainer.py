...
class MergeModelTrainer(ModelTrainer):
    ...
    def train(self, examples, **kwargs):
        ...
        for gi, group in enumerate(optimizer.param_groups):
            print(f"Param group {gi}:")
            for pi, p in enumerate(group["params"]):
                print(f"  param_{pi}: shape={tuple(p.shape)}, requires_grad={p.requires_grad}")
        ...
        for epoch in range(self.hparams.n_epochs):
            for step, batch in enumerate(train_dataloader):
                ...
                for mb in range(num_minibatches):
                    ...
                    # total
                    steer_loss = pos_loss_weight * pos_loss
                    ...
                    if hasattr(self.hparams, "loss_output_dir") and self.hparams.loss_output_dir is not None:
                        ...
                        if not os.path.exists(self.loss_log_file):
                            with open(self.loss_log_file, "w", newline="") as f:
                                writer = csv.writer(f)
                                writer.writerow(["epoch", "step", "pos_steer_loss", "pos_ref_loss", "pos_steering_factors", "token_nums"])
                        ...
                        with open(self.loss_log_file, "a", newline="") as f:
                            writer = csv.writer(f)
                            writer.writerow([epoch, step, pos_outputs_orig.loss.item(), pos_ref_outputs.loss.item(),
                                             pos_minibatch_inputs["steering_factors"], pos_minibatch_inputs["labels"].ne(-100).sum().item()])

                    print(f"steer_loss: %.6f" % (steer_loss.item()))
