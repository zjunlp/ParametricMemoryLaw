    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        for inst in instances:
            for pair in self.preference_pairs:
                ...
                if f"{pair}_winning_target_only_labels" in inst:
                    inst[f"{pair}_winning_target_only_labels"] = torch.cat(
                        (inst[f"{pair}_winning_target_only_labels"], _winning_label_paddings.clone())
                    )
                if f"{pair}_losing_target_only_labels" in inst:
                    inst[f"{pair}_losing_target_only_labels"] = torch.cat(
                        (inst[f"{pair}_losing_target_only_labels"], _losing_label_paddings.clone())
                    )
...
def get_prompt_suffix_length(tokenizer):
    from steer.utils.templates import safe_apply_chat_template
    message_a = [{"role": "user", "content": 'a'}]
    message_b = [{"role": "user", "content": 'b'}]
    tokens_a = safe_apply_chat_template(tokenizer, message_a, tokenize=True, add_generation_prompt=True)
    tokens_b = safe_apply_chat_template(tokenizer, message_b, tokenize=True, add_generation_prompt=True)
    suffix_length = 0
...
def get_target_only_fallback_suffix_tokens(tokenizer) -> int:
...
def build_target_only_labels(tokenizer, prompt, input_ids, labels, raw_target):
    target_only_labels = labels.clone()
    raw_target_text = "" if raw_target is None else str(raw_target)
    ...
    if raw_target_text:
        ...
        target_end_ids = tokenizer(
            prompt + raw_target_text,
            max_length=tokenizer.model_max_length,
            truncation=True,
            return_tensors="pt",
        )["input_ids"][0]
        prompt_length = min(len(prompt_ids), len(target_only_labels))
        target_end_length = min(len(target_end_ids), len(target_only_labels))
        target_only_labels[:prompt_length] = -100
        target_only_labels[target_end_length:] = -100
        if target_only_labels.ne(-100).sum().item() > 0:
            return target_only_labels

    suffix_tokens = get_target_only_fallback_suffix_tokens(tokenizer)
    supervised_positions = labels.ne(-100).nonzero(as_tuple=False).flatten()
    if len(supervised_positions) > suffix_tokens:
        suffix_positions = supervised_positions[-suffix_tokens:]
        target_only_labels[suffix_positions] = -100
    return target_only_labels
...
def preprocess_preference_data(
    ...
    winning_raw_target=None,
    losing_raw_target=None,
):
    ...
    if target_only_loss:
        data["winning_target_only_labels"] = build_target_only_labels(
            tokenizer, prompt, winning_input_ids, winning_output_ids, winning_raw_target
        )
        data["losing_target_only_labels"] = build_target_only_labels(
            tokenizer, prompt, losing_input_ids, losing_output_ids, losing_raw_target
        )
...
def make_preference_data_module(
    ...
    **kwargs
    ...
    target_only_loss = bool(kwargs.get("target_only_loss", False))
    target_only_raw_target_field = kwargs.get("target_only_raw_target_field", "target")
    ...
    for pair in preference_pairs:
        ...
        if target_only_loss:
            all_data[f"{pair}_winning_target_only_labels"] = []
            all_data[f"{pair}_losing_target_only_labels"] = []
    ...
    for item in dataset:        
        ...
        raw_target = item.get(target_only_raw_target_field)
